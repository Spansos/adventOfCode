...
sands = set()
while not sandgen in sands:
    down = (cur_sand[0], cur_sand[1]+1)
    down_left = (cur_sand[0]-1, cur_sand[1]+1)
    down_right = (cur_sand[0]+1, cur_sand[1]+1)
    onfloor = cur_sand[1]+1 == max_y+2
    
    if (not onfloor) and (not down in sands and not down in rockpaths):
        cur_sand[1] += 1
    elif (not onfloor) and (not down_left in sands and not down_left in rockpaths):
        cur_sand[0] -= 1
        cur_sand[1] += 1
    elif (not onfloor) and (not down_right in sands and not down_right in rockpaths):
        cur_sand[0] += 1
        cur_sand[1] += 1
    else:
